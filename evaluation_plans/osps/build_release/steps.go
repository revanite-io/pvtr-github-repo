...
func CicdSanitizedInputParameters(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if len(workflows) == 0 {
		...
		return gemara.NotApplicable, message, 0 // TODO
	}
	...
	for _, file := range workflows {
		...
		if *file.Encoding != "base64" {
			return gemara.Failed, fmt.Sprintf("File %v is not base64 encoded", file.Name), 0 // TODO
		}

		decoded, err := base64.StdEncoding.DecodeString(*file.Content)
		if err != nil {
			return gemara.Failed, fmt.Sprintf("Error decoding workflow file: %v", err), 0 // TODO
		}

		workflow, actionError := actionlint.Parse(decoded)
		if actionError != nil {
			return gemara.Failed, fmt.Sprintf("Error parsing workflow: %v (%s)", actionError, *file.Path), 0 // TODO
		}
		...
		if !ok {
			return gemara.Failed, message, 0 // TODO
		}
	}

	return gemara.Passed, "GitHub Workflows variables do not contain untrusted inputs", 0 // TODO
...
func ReleaseHasUniqueIdentifier(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if noNameCount > 0 || len(sameNameFound) > 0 {
		...
		return gemara.Failed, strings.Join(message, ". "), 0 // TODO
	}
	return gemara.Passed, "All releases found have a unique name", 0 // TODO
}
...
func EnsureInsightsLinksUseHTTPS(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if len(badURIs) > 0 {
		return gemara.Failed, fmt.Sprintf("The following links do not use HTTPS: %v", strings.Join(badURIs, ", ")), 0 // TODO
	}
	return gemara.Passed, "All links use HTTPS", 0 // TODO
}
...
func EnsureLatestReleaseHasChangelog(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if strings.Contains(releaseDescription, "Change Log") || strings.Contains(releaseDescription, "Changelog") {
		return gemara.Passed, "Mention of a changelog found in the latest release", 0 // TODO
	}
	return gemara.Failed, "The latest release does not have mention of a changelog: \n" + releaseDescription, 0 // TODO
}
...
func InsightsHasSlsaAttestation(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	for _, attestation := range attestations {
		if attestation.PredicateURI == "https://slsa.dev/provenance/v1" {
			return gemara.Passed, "Found SLSA attestation in security insights", 0 // TODO
		}
	}
	return gemara.Failed, "No SLSA attestation found in security insights", 0 // TODO
}
...
func DistributionPointsUseHTTPS(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if len(distributionPoints) == 0 {
		return gemara.NotApplicable, "No official distribution points found in Security Insights data", 0 // TODO
	}
	...
	if len(badURIs) > 0 {
		return gemara.Failed,
			fmt.Sprintf("The following distribution points do not use HTTPS: %v", strings.Join(badURIs, ", ")),
			0 // TODO
	}
	return gemara.Passed, "All distribution points use HTTPS", 0 // TODO
}
...
func SecretScanningInUse(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if data.SecurityPosture.PreventsPushingSecrets() && data.SecurityPosture.ScansForSecrets() {
		return gemara.Passed, "Secret scanning is enabled and prevents pushing secrets", 0 // TODO
	} else if data.SecurityPosture.PreventsPushingSecrets() || data.SecurityPosture.ScansForSecrets() {
		return gemara.Failed, "Secret scanning is only partially enabled", 0 // TODO
	} else {
		return gemara.Failed, "Secret scanning is not enabled", 0 // TODO
	}
}
