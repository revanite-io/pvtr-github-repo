...
func FoundLicense(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if data.Repository.LicenseInfo.Url == "" {
		return gemara.Failed, "License was not found in a well known location via the GitHub API", 0 // TODO
	}
	return gemara.Passed, "License was found in a well known location via the GitHub API", 0 // TODO
}
...
func ReleasesLicensed(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	return gemara.Passed, "GitHub releases include the license(s) in the released source code.", 0 // TODO
}
...
func GoodLicense(payloadData any) (result gemara.Result, message string, confidence gemara.ConfidenceLevel) {
	...
	if errString != "" {
		return gemara.Unknown, errString, 0 // TODO
	}

	apiInfo := data.Repository.LicenseInfo.SpdxId
	siInfo := data.Insights.Repository.License.Expression
	if apiInfo == "" && siInfo == "" {
		return gemara.Failed, "License SPDX identifier was not found in Security Insights data or via GitHub API", 0 // TODO
	}
	...
	if len(badLicenses) > 0 {
		return gemara.Failed, fmt.Sprintf("These licenses are not OSI or FSF approved: %s", strings.Join(badLicenses, ", ")), 0 // TODO
	}
	return gemara.Passed, "All license found are OSI or FSF approved", 0 // TODO
}
